---
title: "Managing Tools in DSPy"
description: "Type-Safe Tool Contracts for Multi-Module DSPy Programs"
...
When you compile and save a DSPy program, you're serializing a _constructed instance_, not the class definition. When you load the artifact later, `__init__` is never called again—DSPy unpickles the already-assembled module. Any Protocol passed to the constructor at training time is frozen into the artifact and cannot be replaced at inference.
...
The correct model is that the Protocol defines the program's _shape_ and is fixed at compile time. Only _implementations_ of that Protocol swap between environments. This is why the top-level Protocol belongs in the same source file as the agent definition—versioned together, frozen together, deployed together.
