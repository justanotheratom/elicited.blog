...
> A side note on the word "agent." These three are not agents in the strict sense: no tool use, no loop, no autonomy. They are just predictors in a fixed pipeline, and I am calling them agents to keep the example readable. The point that matters here is that each one is a separate _predictor with its own instruction_. Swap any of them for a real `dspy.ReAct` agent running a tool-use loop and nothing about the GEPA story changes: it is still a named predictor with an instruction, still one key in the candidate dict, still selected and rewritten one component at a time.
