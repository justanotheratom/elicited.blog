...
modDatetime: 2025-12-09T10:00:00Z
author: "Sanket Patel"
featured: false
draft: false
tags:
...
This isn't a comprehensive guide to GEPA. It's the collection of "oh, _that's_ how it works" moments I had while reading through the implementation. The stuff that wasn't obvious from the docs or API surface.

## Quick Primer on GEPA

GEPA (Genetic-Pareto) is a reflective optimizer. It evolves prompts by having an LLM critique failures and propose improvements. The "Pareto" in the name doesn't mean multi-objective optimization. It means the algorithm keeps candidates that are best on _any_ validation example, not just the best on average.
...
**The feedback matters here too.** Even though GEPA only optimizes the scalar score, the reflection LLM reads your feedback text. If you explain _why_ a candidate scored poorly ("too many tokens", "latency exceeded budget"), the reflection can propose targeted improvements. A metric that just returns `0.7` gives less signal than one that returns `0.7` with "accuracy good but 3x over token budget."
...
The GEPA maintainers are also adding native support for objective and hybrid multi-data Pareto frontier tracking (see PR #92), so true multi-objective workflows may not require hand-crafted composite metrics.[^lakshya-feedback]
...
**For single-predictor programs**, use_merge is a no-op.
...
**At selection time:** The generalist wins. GEPA returns the candidate with the highest _average_ score across all validation examples.
...
- **Tie with best score:** Candidate index gets _added_ to that example's frontier
- **Beat best score:** Frontier gets _replaced_. All previous indices removed, only new winner remains
...
| Merge              | Deterministic recombination, no LLM, overlap-gated; single-predictor programs act the same with or without it |
...
3. **Disable merge for single-predictor programs.** Set `use_merge=False`. It won't help and just burns budget (though leaving it enabled doesn't change behavior).
...
Final selection picks the candidate with best _average_ score. Early stopping might catch you when you only have specialists. The generalists emerge from specialists, but that emergence takes time. It's evolution: narrow specialists first, then some generalists emerge that thrive across environments.
...
[^lakshya-feedback]: Clarifications courtesy of GEPA co-author Lakshya A. Agrawal responding to this post.[https://t.co/zstmiOQZ5L](https://t.co/zstmiOQZ5L)
