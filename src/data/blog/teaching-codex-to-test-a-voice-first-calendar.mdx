...
I have been working on [KIN](https://www.kincalendar.com/), a voice-first shared family calendar.
...
![Flow diagram showing the local KIN voice test harness. Codex runs the local runner, FlowDeck launches the simulator, XCUITest holds the voice bar, a helper plays audio through Loopback, KIN records it from the simulator microphone, and the local backend mutates Supabase state.](/assets/kin-loopback-voice-test-flow.svg)
